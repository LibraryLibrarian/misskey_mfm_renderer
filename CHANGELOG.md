...
- **Breaking:** Store ownership moved from `IsarEmojiStore.ownsIsar` to `PersistentEmojiCatalog.ownsStore`, which defaults to `true`. Pass `ownsStore: false` when a store is shared or disposed by the caller
