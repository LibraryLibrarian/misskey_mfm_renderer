...
`0.6.0-beta.2`のようなプレリリースを`0.6.0`として公開する場合も、手順は通常のリリースと同じです。`Prepare Release`のチャンネルに`stable`を選んでください。バージョンは直近の安定版に影響度を適用して決まります。開いているプレリリース列に後から破壊的変更が入った場合は、列のコアより大きい値になります。

このとき`tool/bump_version.dart`は昇格モードで動作します。

- `[Unreleased]`の内容と、直近の安定版より後の全プレリリース（`0.6.0-beta.1` / `0.6.0-beta.2`など）の内容を、カテゴリ別に統合して`## [0.6.0]`に書き出します
